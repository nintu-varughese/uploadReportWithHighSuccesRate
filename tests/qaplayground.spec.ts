...
import RightClickMenuPage from "../pages/rightClickMenuPage";
...
test.describe("QA Playground Full Automation Sequence", () => {
  ...
  let redirectPage: RedirectChainPage;
  let rightClickPage: RightClickMenuPage;
  let fetchingDataPage: FetchingDataPage;
  let changeableIframePage: ChangeableIframePage;
  ...
  test.beforeEach(async ({ page, baseURL }) => {
    ...
    redirectPage = new RedirectChainPage(page);
    rightClickPage = new RightClickMenuPage(page);
    fetchingDataPage = new FetchingDataPage(page);
    changeableIframePage = new ChangeableIframePage(page);
  ...
  test("TC03: Ensure only one Spider-Man row exists", async () => {
    ...
    await expect(
      ...
      "More than one Spider-Man row found"
    ).toHaveCount(1);
  });
  ...
  test("TC04: Spider-Man real name remains correct after shuffle", async () => {
    ...
    await expect(
      miniWebApps.spiderManRealName,
      "Spider-Man real name changed after shuffle"
    ).toHaveText("Peter Parker");
  });
  ...
  test('TC05: Enter code "9" and check success message', async () => {
    ...
    await expect(
      ...
      "Success message not visible after entering correct code"
    ).toBeVisible();
  });
  ...
  test('TC06: Enter incorrect code "1" and verify success message NOT visible', async () => {
    ...
    await expect(
      verifyAccountPage.successMessage,
      "Success message should NOT be visible for incorrect code"
    ).not.toBeVisible();
  });
  ...
  test("TC07: Enter empty code and verify success message not visible", async () => {
    ...
    await expect(
      verifyAccountPage.successMessage,
      "Success message should NOT be visible for empty code"
    ).not.toBeVisible();
  });
  ...
  test("TC08: Add multiple tags and verify counts", async () => {
    ...
    await expect(
      ...
      "Tag count mismatch after adding multiple tags"
    ).toHaveCount(2);
    await expect(
      tagsInputBoxPage.remainingCount,
      "Remaining count incorrect after adding multiple tags"
    ).toHaveText("8");
  });
  ...
  test("TC09: Remove a tag and verify updated count", async () => {
    ...
    await expect(
      tagsInputBoxPage.tags,
      "Tag count mismatch after removing a tag"
    ).toHaveCount(1);
    await expect(
      tagsInputBoxPage.remainingCount,
      "Remaining count incorrect after removing a tag"
    ).toHaveText("9");
  });
  ...
  test("TC10: Add empty tag should not be allowed", async () => {
    ...
    await expect(
      tagsInputBoxPage.remainingCount,
      "Remaining count incorrect after adding empty tag"
    ).toHaveText("10");
  });
  ...
  test("TC12: Adding tags beyond max limit should be prevented", async () => {
    ...
    await expect(
      tagsInputBoxPage.tags,
      "Tag count should not exceed max limit of 10"
    ).toHaveCount(10);
  });
  ...
  test("TC13: Adding invalid/special characters in tag", async () => {
    ...
    await expect(
      tagsInputBoxPage.tags,
      "Special character tags count mismatch"
    ).toHaveCount(2);
  });
  ...
  test('TC15: Open "Animals", verify submenu and click first animal to check URL', async () => {
    ...
    await expect(animalsUrl, "Animals section URL mismatch").toBe(
    ...
    await expect(firstAnimalUrl, "First animal URL mismatch").toBe(
  ...
  test("TC17: Open popup, close it, and verify success message", async () => {
    ...
    await expect(
      ...
      "Success message after closing popup not visible"
    ).toBeVisible();
    await expect(
      successMessageLocator,
  ...
  test("TC22: Download a file and verify name & size", async () => {
    ...
    await expect(path.basename(filePath), "Downloaded file name mismatch").toBe(
      "sample.pdf"
    );
  ...
  test("TC27: Click Boost button in Shadow DOM and verify progress", async ({
    page,
  }) => {
  ...
  test("TC28: Move slider until Send Feedback visible, click it, verify thank you message", async () => {
    ...
    await expect(
      ...
      "Feedback message not visible after clicking Send Feedback"
    ).toBeVisible();
  });
  ...
  test("TC30: Sortable List - Arrange items in correct order", async ({
    page,
  }) => {
    ...
    for (const [index, item] of finalItems.entries()) {
      await expect(
        item.className,
        `Sortable list item at index ${index} is not in correct order`
      ).toContain("right");
    }
  });
  test("TC31: Verify Redirect Chain messages and Go Back button", async ({
    page,
  }) => {
  ...
  test("TC32:Click on each menu and sub-menu item and verify the message", async ({}) => {
    await rightClickPage.rightClickHeader();
    await expect(
      rightClickPage.previewOption,
      "Preview option not visible"
    ).toBeVisible();
    await expect(
    ...
    await expect(
      ...
      "Get Link option not visible"
    ).toBeVisible();
    await expect(
      rightClickPage.renameOption,
      "Rename option not visible"
    ).toBeVisible();
    await expect(
    ...
    await expect(
      ...
      "Telegram not visible"
    ).toBeVisible();
  });
  test("TC33: Verify 100 cards are loaded with headers and body", async () => {
    await fetchingDataPage.clickFetchingDataHeader();
    await fetchingDataPage.page.waitForFunction(
      (selector: string) => document.querySelectorAll(selector).length === 100,
      fetchingDataPage.cardSelector
    );
    const cards = fetchingDataPage.page.locator(fetchingDataPage.cardSelector);
    await expect(cards, "Expected 100 cards to be rendered").toHaveCount(100);
    for (let i = 0; i < 100; i++) {
      const card = cards.nth(i);
      await expect(
        card.locator(fetchingDataPage.cardHeaderSelector),
        `Card ${i + 1} header is not visible`
      ).toBeVisible();
      await expect(
        card.locator(fetchingDataPage.cardBodySelector),
        `Card ${i + 1} body is not visible`
      ).toBeVisible();
    }
  });
  test("TC34: Verify Changeable Iframe sequence", async ({ page }) => {
    ...
    await expect(
      ...
      "Second Iframe legend did not appear after waiting period"
    ).toBeVisible();
  });
});
